...
type ZoomedImage = any;

export default function MediumLightbox(element: string, options: Options = {}) {
  ...
  function zoom() {
    if (!this.isZoomed) {
      ...
      //Get img node
      if (!this.img) this.img = this.getElementsByTagName('img')[0];
      ...
      var that = this;
      setTimeout(function() {
        that.wrapper.style.cssText =
          'transform: translate(' +
          wrapX +
          'px, ' +
          wrapY +
          'px) translateZ(0px);-webkit-transform: translate(' +
          wrapX +
          'px, ' +
          wrapY +
          'px) translateZ(0px);';
        that.img.style.cssText =
          'transform: scale(' +
          scale +
      ...
      setTimeout(function() {
        ...
        if (that.img.classList) that.img.classList.remove(className);
        else
          that.img.className = that.img.className.replace(
            new RegExp(
